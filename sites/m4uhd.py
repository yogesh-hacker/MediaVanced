...
# Constants
base_url = 'https://m4uhd.vip/movies/frankenstein-2025-y9o76'
domain_api = "https://api-play-270325.ppzj-youtube.cfd/api/tp1rd"
...
# Initialize a session to bypass XSRF token checks
session = requests.Session()

# Retrieve the main page content
response = session.get(base_url, headers=headers).text
soup = BeautifulSoup(response, 'html.parser')

# Extract all available servers
servers = {}
server_list = soup.select('div.pick-server')
for server in server_list:
    server_name = server.text.upper()
    server_id = server.select_one('span').get('data')
    servers[server_name] = server_id

# Retrieve selected server ID and security token
server_id = servers.get('SV-YT')
token = re.search(r"_token:\s*\'(.*?)\'", response).group(1)
payload = {
    'm4uhd': server_id,
    '_token': token
}

# Request embed link via AJAX
post_headers = {
    **headers,
    'X-Requested-With': 'XMLHttpRequest'
}
response = session.post(f'{default_domain}/ajax', headers=post_headers, data=payload).text
embed_url = re.search(r'src=\"(.*?)\"', response).group(1)

# Load embed page content
